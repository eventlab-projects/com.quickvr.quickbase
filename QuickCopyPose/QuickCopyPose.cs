...
public class QuickCopyPose : MonoBehaviour
{
    ...
    protected virtual void OnEnable()
    {
        ...
        QuickVRManager.OnPostUpdateTracking += UpdateCamera;
    ...
    {
        if (!_hTrackingMaster) return;

        //_cameraControllerRootLocalPos = _cameraControllerRoot.localPosition;
        //_cameraControllerRootLocalRot = _cameraControllerRoot.localRotation;

        ////Rotate the camera to fit the current avatar look at direction
        //Quaternion rotOffset = Quaternion.FromToRotation(_hTrackingMaster.transform.forward, transform.forward);
        //_cameraControllerRoot.rotation *= rotOffset;

        //if (Vector3.Dot(_hTrackingMaster.GetCamera().transform.up, transform.up) < 0.0f)
        //{
        //    //The view is reversed. Fix it. 
        //    _cameraControllerRoot.rotation = Quaternion.LookRotation(_cameraControllerRoot.forward, transform.up);
        //}

        ////Move the camera to the mid eye point of the avatar. 
        //Vector3 _targetEyesPos = (_targetAnimator.GetBoneTransform(HumanBodyBones.LeftEye).position + _targetAnimator.GetBoneTransform(HumanBodyBones.RightEye).position) / 2.0f;
        //Vector3 posOffset = _targetEyesPos - _hTrackingMaster.GetCamera().transform.position;
        //_cameraControllerRoot.position += posOffset;
    }
